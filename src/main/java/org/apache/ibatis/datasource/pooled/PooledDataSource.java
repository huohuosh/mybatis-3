...
public class PooledDataSource implements DataSource {
   ...
   * 将使用完的连接，添加回连接池中
   * 1.连接如果不可用，坏连接次数加 1
   * 2.连接可用，但空闲连接数已满，回滚事务，关闭数据库连接，当前连接置为无效
   * 3.连接可用，空闲连接数未满，回滚事务，根据 realConnection 创建新的 PooledConnection，
   * 加入空闲连接集合，当前连接置为无效，唤醒正在等待连接的线程
   * @param conn
   * @throws SQLException
   */
      ...
      synchronized (state) {
        ...
        if (!state.idleConnections.isEmpty()) {
          ...
          if (state.activeConnections.size() < poolMaximumActiveConnections) {
            ...
            /**
             * 不能创建新的 connection，获得首个激活的 PooledConnection 对象
             * - 超时
             *   从活跃的连接集合移除
             *   非自动提交，回滚事务
             *   创建连接，老的连接置为无效
  ...
  protected boolean pingConnection(PooledConnection conn) {
    ...
    if (result) {
      // 如果启用侦测查询，判断是否长时间未使用。若是，才需要发起 ping
  ...
  public static Connection unwrapConnection(Connection conn) {
    // 如果传入的是被代理的连接，如果 InvocationHandler 是 PooledConnection，获取真实连接
